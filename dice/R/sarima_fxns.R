	...
	for (iregion in 1:nregion) {
		...
		if (!is.null(arima_model) & covar != FALSE) {
			...
			if (tolower(covar) == "sh") {
				...
				z <- z <- mydata$fit$temp[(nfit+1 - covar_lag):(nperiods - covar_lag), iregion]
			} else if (tolower(covar) == "school") {
				y <- all_years_epi$fit$school[(ifirst):(my_row - covar_lag), iregion]
				z <- z <- mydata$fit$school[(nfit+1 - covar_lag):(nperiods - covar_lag), iregion]
			} else {
				y <- all_years_epi$fit$sh[(ifirst):(my_row - covar_lag), iregion]
				z <- z <- mydata$fit$sh[(nfit+1 - covar_lag):(nperiods - covar_lag), iregion]
			}
			if (!is.null(nadd)) y = c(rep(0, nadd),y)
			y <- na.interp(y)
			y <- log(y + tiny)
