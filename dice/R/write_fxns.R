...
writeCSV <- function(mydata = NULL, run.list = NULL, tab = NULL, tab.model = NULL, model_rtn = NULL, model_profile = NULL, rtn = NULL,
    ...
    #' if appropriate predictions. We write the best result along with the mean
    #' @param tab A list with the history of the MCMC chain for each of the fit_level regions
    #' @param tab.model An array with the history of the MCMC chain for the model_level region
    #' @param model_rtn A 1D numeric array with the best direct prediction to the region
    #' @param model_profile A 2D numeric array with randomly chosen predicted profiles obtained by fitting the mydata
    #' @param mydata - dataframe with all the data for this \pkg{DICE} run
    #' @param ireal Integer - the MCMC chain number
...
saveRData <- function(mydata = NULL, all_years_epi = NULL, run.input = NULL, ireal = 1, subDir = NULL) {
    ...
    #' @param subDir - String sub-directory name for all output files of the run
    #' @return err   \eqn{err =0} if successful.
    #' @examples
    #' saveRData(mydata = mydata, run.input= run.input,ireal = 1, subDir = 'output')
