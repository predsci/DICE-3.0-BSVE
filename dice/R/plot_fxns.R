        ...
        for (iweek in nperiodsFit:min(nperiods, nperiodsFit + 4)) {
            ...
            if (iweek <= nperiodsData) {
                ...
                if (!is.na(model_ili[iweek])) {
                  hist(model_ili[iweek], breaks = breaks, xlim = range(breaks), col = rgb(0, 0, 1, 1/4), border = NULL, xlab = xlab,
                    ylab = ylab, main = "", freq = F)
                  hist(model_profile_ili[, iweek], breaks = breaks, xlim = range(breaks), col = rgb(0, 1, 0, 1/4), border = NULL, xlab = "",
...
plotFitCDCPercentILI <- function(rtn = NULL, profile = NULL, model_rtn = NULL, model_profile = NULL, mydata = NULL, ireal = 1, run.list = NULL, idevice = 1) {
    ...
    plot(1:nperiods, model_ili, type = "l", col = "black", xlim = c(1, nperiods), ylim = c(0, ymax), xaxt = "n", xlab = "EW #", ylab = "% ILI",
        lwd = 2, lty = 1)
    points(1:nperiodsFit, model_ili[1:nperiodsFit], type = "p", col = "black", pch = 20, xaxt = "n", xlab = "", ylab = "")
...
plotHists <- function(rtn = NULL, profile = NULL, model_rtn = NULL, model_profile = NULL, mydata = NULL, ireal = 1, run.list = NULL,
    ...
    for (iregion in 1:nregions) {
        ...
        for (iweek in nperiodsFit:min(nperiods, nperiodsFit + 4)) {
            ...
            if (iweek <= nperiodsData) {
                ...
                hist(profile_ili[, iweek, iregion], breaks = breaks, xlim = range(breaks), col = rgb(1, 0, 0, 1/4), border = NULL, xlab = "",
                  ylab = "", main = "", freq = F)
                if (!is.na(fit_ili[iweek, iregion]))
                  hist(fit_ili[iweek, iregion], breaks = breaks, xlim = range(breaks), col = rgb(0, 0, 1, 1/4), border = NULL, xlab = xlab,
    ...
    for (iweek in nperiodsFit:min(nperiods, nperiodsFit + 4)) {
        ...
        if (iweek <= nperiodsData) {
            ...
            max.val = 2 * max.val
            breaks = seq(from = min.val, to = max.val, by = 0.5)
            hist(model_ili[iweek], breaks = breaks, xlim = range(breaks), col = rgb(0, 0, 1, 1/4), border = NULL, xlab = xlab, ylab = ylab,
                main = "", freq = F)
            hist(model_profile_indr[, iweek], breaks = breaks, xlim = range(breaks), col = rgb(1, 0, 0, 1/4), border = NULL, xlab = "",
                ylab = "", main = "", freq = F, add = TRUE)
            leg.text = c(mydata$FY)
            model.name = mydata$model$name
            my.week = paste("%ILI for EW # ", weeks[iweek], sep = "")
    ...
    if (!is.null(model_profile_ili)) {
        for (iweek in nperiodsFit:min(nperiods, nperiodsFit + 4)) {
            ...
            if (iweek <= nperiodsData) {
                ...
                max.val = ceiling(max(model_profile_ili[, iweek], na.rm = TRUE))
                max.val = 2 * max.val
                cat("model", iweek, max.val, range(model_profile_ili[, iweek]), "\n")
                breaks = seq(from = min.val, to = max.val, by = 0.5)
                hist(model_profile_ili[, iweek], breaks = breaks, xlim = range(breaks), col = rgb(0, 1, 0, 1/4), border = NULL, xlab = "",
                  ylab = "", main = "", freq = F)
...
plotDisease <- function(mydata=NULL, all_years_epi = NULL, run.list = NULL, device = NULL) {
    ...
    #' plotDisease(mydata = mydata, all_years_epi = all_years_epi, PDF = TRUE)
...
#   if (mydata$fit_level > mydata$mod_level) {
   if (nregions > 1) {
