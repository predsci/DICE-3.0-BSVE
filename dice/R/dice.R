...
runDICE <- function(data_source = NULL, year = 2016, mod_level = 2, fit_level = 3, nfit = 52, model = 5, isingle = 0, nMCMC = 1e+05, nreal = 1, device = "pdf", prior = 0, Temp = 1, da = 0, mod_name=c(NAME_2="US"), RegState=NULL, fit_names="all", subDir = NULL, plot = 1, iseed = NULL, Tg = NULL, epi_model = NULL, disease='flu', db_opts=list(DICE_db="predsci", CDC_server=TRUE), arima_model = NULL, method = 'mech', covar = FALSE, covar_lag = 1) {
  ...
  if (is.null(Tg)) {
    Tg = 3.0
    if(tolower(disease) == 'dengue' ) Tg = 8.0

    if(tolower(disease) == 'yellow_fever' ) Tg = 5.0

    if(tolower(disease) == 'plague' ) Tg = 5.0

    if(tolower(disease) == 'chik' )  Tg = 8.0
...
fitOnePatch <- function(mydata = NULL, all_years_epi = NULL, opt.list = NULL, run.list = NULL, ireal = 1, iseed = NULL) {
	...
	if (tolower(mydata$disease) == "dengue" || tolower(mydata$disease) == "yellow_fever") {
		...
		epi.da.df = get.sql.da(nfit = nperiodsFit, years = all_years_epi$years, mydata = mydata, cases = all_years_epi$model$epi, epi = mydata$model$epi,
    ...
    success = saveRData(mydata = mydata, all_years_epi = all_years_epi, run.input = run.input, ireal = ireal, subDir = run.list$subDir)
	...
	if (tolower(mydata$disease) == "dengue") {
		...
		if (fit_level > mod_level) {
			...
			for (iregion in 1:mydata$fit$nregions) {
				...
				epi.da.df = get.sql.da(nfit = nperiodsFit, years = all_years_epi$years, mydata = mydata, cases = all_years_epi$fit$epi[, iregion], epi = mydata$fit$epi[, iregion], corrMAT = corrMAT, distMAT = distMAT, deng.null = epi.null.fit, my_id = fit_id[iregion])
