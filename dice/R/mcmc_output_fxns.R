...
mcmc.onepatch.plot <- function(tab.model = NULL, opt.list = NULL, run.list = NULL, mydata = NULL, imask = NULL, ireal = 1, idevice = 1) {
    ...
    if (!is.null(tab.model)) {
        ...
        colnames(tab.model) = c(myColName, "LLK")
...
mcmc.multi.write <- function(tab.model = NULL, tab = NULL, tab.cpl = NULL, opt.list = NULL, opt.cpl = NULL, run.list = NULL, mydata = NULL,
    ...
    for (i in 1:n.fit) {
        ...
        colnames(tab[, , i]) = c(myColName, "AICc")
        tab[, (nparam + 1), i] <- 2 * tab[, (nparam + 1), i] + 2 * nopt
        tab[, (nparam + 1), i] <- tab[, (nparam + 1), i] + (2 * nopt * (nopt + 1))/(nperiods - nopt - 1)
    }
	...
	dump = list()
    dump = list(mydata = mydata, tab.model = tab.model, tab.fit = tab, tab.cpl = tab.cpl, opt.list = opt.list, opt.cpl = opt.cpl, run.list = run.list, imask = imask, cpl_imask = cpl_imask, ireal = ireal, idevice = 1)
