...
get.DICE.data <- function(data_source = "cdc", mod_level = 2, mod_name=c(NAME_2="US"), fit_names="all", fit_level = 3, RegState=NULL, year = 2015, nperiodsFit = 52, model = 4, isingle = 0, db_opts=list(DICE_db="predsci", CDC_server=TRUE), disease="flu", epi_model = 1, method ='mech', all_years_flag=T, all_cad_clim=T) {
  ...
  if (fit_level < mod_level)
    ...
    mysql_out = get.mysql(mod_level=mod_level, fit_level=fit_level, mod_name=mod_name, fit_names=fit_names, disease=disease, sql_data_source=data_source, season=year, all_years_flag=all_years_flag, db_opts=db_opts, cad_clim=TRUE)
  ...
  if (method == "mech") {
    if (isingle == 0)
      mydata$dataName = paste0(data_source, "-", epi_name, "-", mydata$model$name, "-cpl-", mydata$FY, "-", mydata$imodel)
    if (isingle == 1)
      mydata$dataName = paste0(data_source, "-", epi_name, "-", mydata$model$name, "-uncpl-", mydata$FY, "-", mydata$imodel)
    if (fit_level == mod_level)
      mydata$dataName = paste0(data_source, "-", epi_name, "-", mydata$model$name, "-", mydata$FY, "-", mydata$imodel)
  } else {
    mydata$dataName = paste0(data_source, "-", mydata$model$name, "-", mydata$FY)
  }
...
get.mysql <- function(mod_level=2, fit_level=3, mod_name=c(NAME_2 = "BR"), fit_names="all", start.date=as.Date("2010-01-01"), end.date=as.Date("2010-12-31"), disease="dengue", sql_data_source=NULL, season=NULL, all_years_flag=T, db_opts=list(DICE_db='predsci', CDC_server=F), cad_clim=TRUE) {
  ...
  if (mod_level>=2) {
    ...
    if (!is.null(fit_names) && (length(fit_names)>1 || fit_names!="all")) {
      mod_sum = TRUE
      fit_ind = fit_ind & (all_years$lut[, paste0("NAME_", fit_level)] %in% fit_names | all_years$lut[, paste0("ABBV_", fit_level)] %in% fit_names)
    } else {
      mod_sum = FALSE
    }
  } else if (mod_level==1) {
  ...
  mydata = Proc_IncCol_BySource_mydata(mydata=mydata, data_source=sql_data_source, cadence=cadence, mod_sum=mod_sum)
  ...
  if (mod_sum) {
    ...
    if (mydata$fit$raw_units=="# of cases") {
      sum_cols = c(sum_cols, "raw")
    } else if (mydata$fit$raw_units=="incidence rate (%)") {
  ...
  if (mydata$fit$raw_units=="incidence rate (%)") {
    mydata$fit$coef = mydata$fit$pop/sum(mydata$fit$pop)
  } else {
  ...
  if (all_years_flag) {
    ...
    for (ii in 1:mydata$fit$nregions) {
      fit_ident = mydata$fit$attr$identifier[ii]
      all_years[[cad_name]][[fit_ident]][[source_abbv]] = Proc_IncCol_BySource(ident_data=all_years[[cad_name]][[fit_ident]][[source_abbv]], data_source=sql_data_source, factor=mydata$fit$factor[ii])
    ...
    if (!mod_sum) {
      # add epi vector
      all_years[[cad_name]][[mod_ident]][[source_abbv]] = Proc_IncCol_BySource(ident_data=all_years[[cad_name]][[mod_ident]][[source_abbv]], data_source=sql_data_source, factor=mydata$model$factor)
...
Proc_IncCol_BySource <- function(ident_data=NULL, data_source=NULL, factor=NULL) {
...
Proc_IncCol_BySource_mydata <- function(mydata=NULL, data_source=NULL, cadence=NULL, mod_sum=NULL) {
  # for each data source, write a procedure that determines which data column to use and converts it to either 'cases' or 'percent'.  Then designate the appropriate column with a matching column name.
  ...
  if (data_source %in% c(7,11)) {
    raw_name = "ILI"
    if (!mod_sum) {
      mydata$model$raw_units = "incidence rate (%)"
      mydata$model$factor = mydata$model$pop*2/100/nperiodsPerYear
    }
    mydata$fit$raw_units = "incidence rate (%)"
    mydata$fit$factor = mydata$fit$pop*2/100/nperiodsPerYear
  } else {
    if (data_source %in% c(26)) {
      raw_name = "total_specimens"
    } else {
      raw_name = "cases"
    }
    ...
    if (!mod_sum) {
      mydata$model$raw_units = "# of cases"
      mydata$model$factor = 1
    }
    mydata$fit$raw_units = "# of cases"
    mydata$fit$factor = rep(1, nrow(mydata$fit$attr))
  }
  ...
  if (data_source == 15) {
    if (!mod_sum) {
      ...
      if (n_ili_cases>n_ili_out) {
        mydata$model[[mod_ident]]$raw = mydata$model[[mod_ident]]$cases
      } else if (n_ili_cases<n_ili_out) {
        mydata$model[[mod_ident]]$raw = mydata$model[[mod_ident]]$ili_out
      } else {
        # sum both columns and use the one with more cases
    ...
    for (ii in 1:length(fit_ident)) {
      ...
      if (n_ili_cases>n_ili_out) {
        mydata$fit[[fit_ident[ii]]]$raw = mydata$fit[[fit_ident[ii]]]$cases
      } else if (n_ili_cases<n_ili_out) {
        mydata$fit[[fit_ident[ii]]]$raw = mydata$fit[[fit_ident[ii]]]$ili_out
      } else {
    ...
    # for all other data sources
    if (!mod_sum) {
      # grab the appropriate column and copy it into 'raw'
  ...
  if (GoodPull_fit) {
    ...
    if (nregions > 1) {
      if (fit_level==4) {
        ...
        tempDate = CDCfit$week_start[sample_ind]
      } else {
        tempYear = CDCfit$year[seq(from=1, to=length(CDCfit$year), by=nregions)]
        tempWeek = CDCfit$week[seq(from=1, to=length(CDCfit$year), by=nregions)]
      }
      ...
      if (fit_level == 4) {
        ...
        for (ii in 1:nregions) {
          ...
          match_ind = match(x=temp_fit$week_start, table=tempDate)
          FitILI[ii, match_ind] = temp_fit$unweighted_ili
          FitCases[ii, match_ind] = temp_fit$ilitotal
          StateTotal[ii, match_ind] = temp_fit$total_patients
        }
        StateILITOTAL = t(FitCases)
        StateTotal    = t(StateTotal)
        # Then re-order to match mydata$fit$attr
        # FitILI = FitILI[req_names, ]

        # FitCases = array(data = CDCfit$ilitotal, dim = c(nregions, length(CDCfit$year)/nregions), dimnames = list(unique(CDCfit$region), NULL))
        # # Then re-order to match mydata$fit$attr
        # FitCases = FitCases[req_names, ]
      } else {
