...
get.future.similar.data <- function(mydata = NULL) {
    ...
    for (iyear in 1:nyears) {
        ...
        pastdata = pastdata$mydata
        for (iregion in 1:nregion) {
            corrMat[iyear, iregion] = cor(fit_raw[1:nperiodsDataUse, iregion], pastdata$fit$raw[1:nperiodsDataUse, iregion], method = "pearson")
        }
        corrMat[iyear, nregion1] = cor(model_raw[1:nperiodsDataUse], pastdata$model$raw[1:nperiodsDataUse], method = "pearson")
...
get.future.average.data <- function(mydata = NULL) {
    ...
    for (iregion in 1:nregion) {
        ...
        my.cor[iregion] = cor(mydata$fit$raw[1:nperiodsDataUse, iregion], hstrc.ave[1:nperiodsDataUse, iregion], method = "pearson")
    }

    shft[nregion1] = mydata$model$raw[nperiodsDataUse] - hstrc.ave[nperiodsDataUse, nregion1]
    my.cor[nregion1] = cor(mydata$model$raw[1:nperiodsDataUse], hstrc.ave[1:nperiodsDataUse, nregion1], method = "pearson")
    ...
    for (i in 1:nregions) {
    	...
    	for (j in 1:nperiods) {
    		...
    		if (nregions > 1) {
    			...
    			cases.ave.fit[j, fit_id[i]] = mean(past.cases.fit[ind])
    		}
    	}
